...
# name tag for data, I use the day I submitted the CRAB jobs
parser.add_argument('--n', dest='DATE')
# above is only option you need to set, it will run everything for 2016

# 2016, 2016APV, 2017, 2018 ONLY
parser.add_argument('--y', dest='YEAR', default='2016')
# if you set ARGS.SELECTED to a name from the keys of the nano_files_201X.py
...
for key, item in SAMPLES.items():
    ...
    if len(dataset) > 1:
        # if there is more than 1 file location or dataset in the letter then
        ...
        if len(dataset) > 2:
            if 'USER' in splits:
                ...
                # if there is more than 1 dataset, eg. if there are extensions
                # to the MC, then usually
                #   - 1st in list is the dataset name
                #   - 2nd entry is basically a duplicate, it is the file
                #     location of that same datset
                #   - 3rd entry is the location of the extension files, usually
                #     asparker cms eos space bc we created those ourselves, so
...
'''
    ...
    if not published:
        if len(ARGS.DATASET) == 1:
           ...
           ARGS.DATASET = df
           print(ARGS.DATASET)
        # now that we have the input files, we can make the file input part of
        # the CRAB cfg file

        CRAB_CFG_UNPUB = '''config.section_('Data')
...
'''
    ...
    if published:
        ARGS.DATASET = dataset[0]
        CRAB_CFG_PUB = '''config.section_('Data')
...
'''
    ...
    CRAB_CFG3 = '''# config.Data.inputDBS = 'phys03'
config.Data.inputDBS = 'global'
config.Data.outLFNDirBase = '/store/user/%s/LFV_%s/%s' % ('etsai', '{DATE}', idname)
...
'''
    ...
    # open crabConfig.py, substitute into CRAB_CFG the arguments from ARGS,
    # write it, run it, and remove it
...
'''
    ...
    CRAB_MC = '''jmeCorrections = createJMECorrector(True, '{ERA}', '{LETTER}', 'Total', 'AK4PFchs')
modulesList.append(jmeCorrections())
modulesList.append(btagSFUL{YEAR}())
'''

    CRAB_DATA = '''jmeCorrections = createJMECorrector(False, '{ERA}', '{LETTER}', 'Total', 'AK4PFchs')
modulesList.append(jmeCorrections())
'''

    CRAB_END = '''p = PostProcessor('.',
                  inputFiles(),
                  cut,
                  modules=modulesList,
                  provenance=True,
...
for aid in ids:
    ARGS.ID = aid
    # idname%s = '%s'
    subCommand = '''print('Submitting CRAB job for dataset: %s')
